...
int64_t hook(uint32_t reserved ) {
    ...
    {
        rollback(SBUF("Swap: Ammount not equal to Requirement "), 2);
    }
    ...
    TRACEVAR(seq_slot);
    if (seq_slot < 0) {
        TRACESTR("Accept: Could not find sfMintedNFTokens on hook account");
    } else {
        uint8_t seq_buf[4];
        seq_slot = slot(SBUF(seq_buf), seq_slot);
        if (seq_slot != 4)
            rollback(SBUF("Accept: Could not fetch sequence from sfMintedNFTokens."), 80);
    ...
    etxn_reserve(1 * 2); // we are going to emit 2 transactions
    // for (int i = 0; GUARD(10), i < num_tickets; ++i)
    // {
    //     if (i <= num_tickets) {
    //         TRACEVAR(i);
    //     }
